...
public class Ex_02_static_1 {
	...
	public static void main(String[] args) {
		...
		// 결론
		// 해당 클래스에서 static으로 전역으로 선언하면
		// 다른 인스턴스에서 해당 전역 변수에 접근시 해당 데이터가 공유가 됨
		// a1 에서 전역변수를 변경하고
		// 다른 인스턴스인 a2에서 접근을 하니 전역변수가 변경됨을 확인함
		
		// 1) 인스턴스 변수로 해당 클래스 변수에 접근이 가능하지만
		// 클래스 변수로는 인스턴스 변수에 접근이 불가
		// 인스턴스는 각각의 고유한 독립값이기 때문
		// 클래스 변수는 해당 클래스가 로더에 의해서 로드가 되면
		// 그때 존재하는 변수 -> 전역 변수
